...
namespace AzureDiscovery.Services
{
    public class ResourceDependencyAnalyzer
    {
        ...
        public ResourceDependencyAnalyzer(ILogger logger)
        {
            _logger = logger;
            ...
            try
            {
                    ...
                    networkProfile.TryGetProperty("networkInterfaces", out var networkInterfaces))
                {
                    foreach (var nic in networkInterfaces.EnumerateArray())
                    {
                        ...
                        {
                            var nicResource = FindResourceById(allResources, nicId.GetString());
                            if (nicResource != null)
                            {
                ...
                if (properties.TryGetProperty("storageProfile", out var storageProfile))
                {
                    ...
                    if (storageProfile.TryGetProperty("osDisk", out var osDisk) &&
                        osDisk.TryGetProperty("managedDisk", out var managedDisk) &&
                        managedDisk.TryGetProperty("id", out var diskId))
                    {
                        var diskResource = FindResourceById(allResources, diskId.GetString());
                    ...
                    if (storageProfile.TryGetProperty("dataDisks", out var dataDisks))
                    {
                        ...
                        {
                            if (dataDisk.TryGetProperty("managedDisk", out var dataManagedDisk) &&
                                dataManagedDisk.TryGetProperty("id", out var dataDiskId))
                            {
                                var dataDiskResource = FindResourceById(allResources, dataDiskId.GetString());
                                if (dataDiskResource != null)
                                {
                                    dependencies.Add(CreateDependency(vm.Id, dataDiskResource.Id, DependencyType.StorageDependency));
                                }
                            }
                        }
                    }
        ...
        private List<ResourceDependency> AnalyzeNetworkInterfaceDependencies(AzureResource nic, List<AzureResource> allResources)
        {
            ...
            try
            {
                ...
                if (properties.TryGetProperty("ipConfigurations", out var ipConfigurations))
                {
                    foreach (var ipConfig in ipConfigurations.EnumerateArray())
                    {
                        if (ipConfig.TryGetProperty("properties", out var ipConfigProps) &&
                            ipConfigProps.TryGetProperty("subnet", out var subnet) &&
                            subnet.TryGetProperty("id", out var subnetId))
                        {
                            // Extract VNet ID from subnet ID
                            var subnetIdStr = subnetId.GetString();
                            var vnetId = ExtractVNetIdFromSubnetId(subnetIdStr);
                            
                            var vnetResource = FindResourceById(allResources, vnetId);
                        ...
                        if (ipConfig.TryGetProperty("properties", out var ipConfigProps2) &&
                            ipConfigProps2.TryGetProperty("publicIPAddress", out var publicIp) &&
                            publicIp.TryGetProperty("id", out var publicIpId))
                        {
                            var publicIpResource = FindResourceById(allResources, publicIpId.GetString());
                ...
                {
                    var nsgResource = FindResourceById(allResources, nsgId.GetString());
                    if (nsgResource != null)
                    {
        ...
        private List<ResourceDependency> AnalyzeStorageAccountDependencies(AzureResource storage, List<AzureResource> allResources)
        {
            ...
            try
            {
                ...
                if (properties.TryGetProperty("encryption", out var encryption) &&
                    encryption.TryGetProperty("keySource", out var keySource) &&
        ...
        private List<ResourceDependency> AnalyzeWebAppDependencies(AzureResource webApp, List<AzureResource> allResources)
        {
            ...
            try
            {
                ...
                {
                    var appServicePlan = FindResourceById(allResources, serverFarmId.GetString());
                ...
                // Storage Account dependencies (for function apps)
                if (properties.TryGetProperty("siteConfig", out var siteConfig) &&
                    siteConfig.TryGetProperty("appSettings", out var appSettings))
                {
                    foreach (var setting in appSettings.EnumerateArray())
                    {
                        if (setting.TryGetProperty("name", out var name) &&
                            setting.TryGetProperty("value", out var value))
                        {
                            var settingName = name.GetString();
                            var settingValue = value.GetString();

                            if (settingName == "AzureWebJobsStorage" || settingName == "WEBSITE_CONTENTAZUREFILECONNECTIONSTRING")
                            {
                                var storageAccount = FindStorageAccountByConnectionString(allResources, settingValue);
                                if (storageAccount != null)
                                {
                                    dependencies.Add(CreateDependency(webApp.Id, storageAccount.Id, DependencyType.StorageDependency));
                                }
                            }
                        }
                    }
        ...
        private List<ResourceDependency> AnalyzeGenericDependencies(AzureResource resource, List<AzureResource> allResources)
        {
            ...
            try
            {
                ...
                foreach (Match match in matches)
                {
                    ...
                    var referencedResource = FindResourceById(allResources, referencedResourceId);
        ...
        private AzureResource? FindResourceById(List<AzureResource> resources, string? resourceId)
        {
            if (string.IsNullOrEmpty(resourceId)) return null;
            
            return resources.FirstOrDefault(r => r.Id.EndsWith(resourceId) || resourceId.EndsWith(r.Id.Split('_').Last()));
        }
        ...
        private AzureResource? FindStorageAccountByConnectionString(List<AzureResource> resources, string? connectionString)
        {
            if (string.IsNullOrEmpty(connectionString)) return null;
            
            var accountName = ExtractStorageAccountNameFromConnectionString(connectionString);
        ...
        private string ExtractStorageAccountNameFromConnectionString(string? connectionString)
        {
            if (string.IsNullOrEmpty(connectionString)) return string.Empty;
            
            var accountNameMatch = Regex.Match(connectionString, @"AccountName=([^;]+)", RegexOptions.IgnoreCase);
            return accountNameMatch.Success ? accountNameMatch.Groups[1].Value : string.Empty;
        }
    }
